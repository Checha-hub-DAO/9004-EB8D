...
function Deploy-C07 {
  ...
  if (-not (Test-Path $Root)) { Fail ("Root not found: {0}" -f $Root) }
  if (-not (Test-Path $Zip))  { Fail ("ZIP not found: {0}" -f $Zip) }
  ...
  if (Test-Path $c07Path) {
    ...
    Write-Host ("Backing up existing C07 -> {0}" -f $backupZip)
  ...
  Write-Host ("Deploying C07 from ZIP: {0}" -f $Zip)
  ...
  Ok ("C07 deployed to {0}" -f $c07Path)

  if (Test-Path (Join-Path $Root ".git")) {
    if ($GitCommit) {
      Push-Location $Root
      git add C07 | Out-Null
      git commit -m "C07 baseline v1.0 (2025-08-18) - deploy" | Out-Null
      Pop-Location
...
function Publish-C07-WithAlias {
  ...
  Write-Host ("Ensuring bucket {0}/{1} (ignore if no permission)" -f $Alias, $Bucket)
  ...
  $volB = "$alt`:/data"
  Write-Host ("Retrying with alternative mount: {0}" -f $volB)
...
if ($PublishAfterDeploy) {
  Info ("Publishing after deploy using alias '{0}' to bucket '{1}'..." -f $PublishAlias, $PublishBucket)
