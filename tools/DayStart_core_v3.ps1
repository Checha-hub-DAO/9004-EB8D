# DayStart_core_v3.ps1 — hardened Day Start for CHECHA_CORE
# - Auto checkpoint if no backups today
# - Robust backup counting
...
$bkCount = @($bk).Count    # robust even if $bk is $null or empty
...
if ($bkCount -eq 0) {
  ...
  # Update in-memory collections to avoid re-reading file
  $ev += [pscustomobject]@{
    DT = Get-Date($ts)
    T  = $ts.Split()[1]
    M  = 'c05-archive'
...
if (Test-Path $rep) {
  ...
  # Find today's section start and end (next Day Start header or EOF)
  ...
  if ($mStart.Success) {
    ...
    # Match individual "Update" 2-line blocks (dash can be — or -)
