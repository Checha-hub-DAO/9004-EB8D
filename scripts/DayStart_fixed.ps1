# DayStart.ps1 — ASCII-safe, UTF-8 BOM. Tested on Windows PowerShell 5.1+
# This script is idempotent. It creates Day Start header, logs start, updates KPI and health-summary.
...
if(-not (Test-Path $log) -or -not (Select-String -Path $log -Pattern $rxT3 -Quiet)){
...
# Last restore-drill (latest in LOG)
...
# 6) health-summary — rebuild (events + 2 tables)
