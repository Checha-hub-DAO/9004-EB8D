...
$redKeywords  = @("deadline","blocked","risk","incident","urgent","critical","sev","breach","failure","outage")
$yelKeywords  = @("dependency","review needed","pending approval","waiting","hold","defer","follow up","remind")
...
$md += ""
$md += "## TOP-" + $MaxTop + " (per-module cap = 2)"
if ($top.Count -gt 0) {
...
$md += "- Module bonuses: C06=+2, C07=+2, G43=+1"
...
# Log entry
$logEntry = "| $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') | c11_focus_auto | OK | [AUTO] focus logic-plus v2 (candidates=" + $items.Count + ", top=" + $top.Count + ", backlog=" + $back.Count + ")"
Add-Content -Path $logPath -Value $logEntry -Encoding UTF8

Write-Host "FOCUS_AUTO.md created (logic-plus v2) and log updated."
